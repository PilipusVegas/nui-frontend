...
const offsetCoordinate = (lat, lng, index) => {
  ...
  const dLng =
    (meters / (111320 * Math.cos((lat * Math.PI) / 180))) * Math.sin(angle);
...
/* PROFILE GRAPHOPPER BERDASARKAN KATEGORI KENDARAAN */
const getGraphHopperProfile = (vehicleCategory) => {
  if (Number(vehicleCategory) === 2) return "car";
  if (Number(vehicleCategory) === 1) return "motorcycle";
  return "motorcycle"; // fallback aman
};
...
const MapRouteMulti = ({
  ...
  useEffect(() => {
    ...
    const points = sorted.reduce((acc, l) => {
      ...
      const visualCoord =
        index === 0 ? coord : offsetCoordinate(coord.lat, coord.lng, index);
    ...
    const fetchRoutes = async () => {
      ...
      const profile = getGraphHopperProfile(vehicleCategory);
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        try {
          const url =
            `${GH_BASE_URL}/maps/route` +
            `?point=${a.originalLat},${a.originalLng}` +
            `&point=${b.originalLat},${b.originalLng}` +
            `&profile=${profile}` +
          ...
          const coords = path.points.coordinates.map(([lng, lat]) => [
            lat,
            lng,
          ]);
    ...
    fetchRoutes();
  }, [locations, vehicleCategory, GH_BASE_URL, onDistanceCalculated]);
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden relative">
      <MapContainer
        center={[orderedPoints[0].lat, orderedPoints[0].lng]}
        zoom={13}
        minZoom={3}
        preferCanvas={true}
        zoomControl={false}
