...
const FooterMainButton = ({ icon, label, to, activePath }) => {
  ...
  return (
    <button onClick={() => navigate(to)} className="group relative flex flex-col items-center justify-center text-white transition-all duration-300 px-3">
      <div className={`absolute w-9 h-9 rounded-full transition-all duration-300 ease-in-out z-0 ${ isActive ? "bg-white scale-125 -translate-y-8 border-2 border-green-600/90" : "scale-0"}`}></div>
      <div className={`relative z-10 flex items-center justify-center w-8 h-8 text-lg transition-all duration-300 ease-in-out ${ isActive ? "text-green-600 -translate-y-5 group-hover:text-green-800" : "text-white/80 group-hover:text-white"}`}>
        <FontAwesomeIcon icon={icon} />
      </div>
      <span className={`relative z-10 text-[11px] font-medium mt-2 tracking-widest transition-all duration-300 ${ isActive ? "-translate-y-5 text-white" : "-translate-y-2 text-white/80 group-hover:text-white" }`}>
        {label}
      </span>
    </button>
  );
