    ...
    <MobileLayout title="Menu">
      <div className="min-h-screen py-6 px-4 bg-white rounded-xl"> 
        ...
        <div className="grid grid-cols-4 gap-5"> 
          ...
          <ActionButton icon={faPenFancy} label="Form" onClick={() => navigate("/form")} color="text-blue-500"/>
          <ActionButton icon={faBell} label="Notifikasi" onClick={() => navigate("/notification")} color="text-yellow-500"/>
          <ActionButton icon={faUser} label="Profil" onClick={() => navigate("/profile")} color="text-blue-500"/>
          <ActionButton icon={faHistory} label="Riwayat" onClick={() => navigate("/riwayat-absensi")} color="text-teal-600"/>
        </div>
      </div>
    </MobileLayout>
  );
...
const ActionButton = ({ icon, label, onClick, color }) => (
  <button onClick={onClick} aria-label={label} className="rounded-md focus:outline-none transition">
