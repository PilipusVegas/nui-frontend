...
const DetailKunjungan = () => {
  ...
  const handleApprovalWithValidation = async (status) => {
    ...
    if (jarak === 0 && data.lokasi?.length > 1) {
      toast.error(
        "Perhitungan jarak perjalanan masih diproses. Mohon tunggu beberapa saat.",
      );
      return;
    }
    ...
    if (isSedangBerangkat()) {
      const result = await Swal.fire({
        ...
        confirmButtonText:
          status === 1 ? "Ya, Tetap Setujui" : "Ya, Tetap Tolak",
  ...
  const routeLocations = React.useMemo(() => {
    ...
    if (rumah) {
      result.push({
        ...rumah,
        jam_mulai:
          visitedLocations[visitedLocations.length - 1]?.jam_selesai ??
          visitedLocations[visitedLocations.length - 1]?.jam_mulai,
      });
    }
    return result;
  }, [data]);
  ...
  const getKategoriKendaraan = (kategori) => {
    return KENDARAAN_KATEGORI_MAP[kategori] || "Tidak diketahui";
  };
        ...
        <div className="p-4 space-y-3">
          ...
          <InfoGrid>
            ...
            <InfoItem
              ...
              value={formatFullDate(data.tanggal)}
            />
          </InfoGrid>
          <Divider />
          ...
          {data.kendaraan && (
            <>
              ...
              <InfoGrid>
                <InfoItem
                  label="Kendaraan"
                  value={`${data.kendaraan.nama_kendaraan} (${data.kendaraan.tahun_kendaraan})`}
                />
                <InfoItem
                  label="Kategori Kendaraan"
                  value={getKategoriKendaraan(data.kendaraan.kategori)}
                />
                <InfoItem label="Jenis BBM" value={data.kendaraan.nama_bb} />
                <InfoItem
                  label="Konsumsi BBM"
                  value={`${data.kendaraan.konsumsi_bb} km / liter`}
                />
                <InfoItem
                  label="Harga BBM"
                  value={formatRupiah(data.kendaraan.harga_bb)}
                />
              </InfoGrid>
              <Divider />
            </>
          ...
          <InfoGrid>
            <InfoItem
              label="Total Jarak (Map)"
              value={formatMeter(totalJarakMap ?? data.total_jarak)}
              highlight
            />
            <InfoItem
              label="Jumlah Lokasi"
              value={`${data.lokasi?.length || 0} titik`}
            />
          ...
          {data?.dinas?.status && (
            <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="text-sm text-blue-900">
                ...
                <p>
                  {" "}
                  Pada tanggal ini karyawan sedang menjalankan{" "}
                  <b>tugas dinas resmi</b>. Oleh karena itu,{" "}
                  <b>tunjangan perjalanan dan klaim BBM tidak berlaku</b>.
                  Karyawan hanya menerima <b>tunjangan dinas</b> sesuai
                  permohonan dinas yang telah disetujui.
                </p>
              </div>
              <button
                onClick={() => navigate(`/permohonan-dinas/${data.dinas.id}`)}
                className="px-3 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 whitespace-nowrap"
              >
                Lihat Detail Dinas
              </button>
            </div>
          ...
          <div className="mt-3 text-sm text-gray-600 space-y-1">
            ...
            <ol className="list-decimal ml-4 space-y-1">
              <li>
                ...
                Namun, apabila aktivitas hanya di kantor saja, maka total
                nominal akan tetap 0 meskipun kunjungan disetujui atau ditolak.
              </li>
              <li>
                Klaim BBM dihitung berdasarkan konsumsi BBM per liter dengan
                rumus:
                <br />
                <span className="italic">
                  Klaim BBM (Rp) = (Jarak tempuh / Standar km per liter) × Harga
                  BBM per liter + 46%
                </span>
              </li>
            </ol>
              ...
              <div className="order-1 lg:order-2 flex flex-col h-[220px] lg:h-full">
                ...
                <div className="flex-1 rounded-lg overflow-hidden border z-10">
                  <MapRouteMulti
                    ...
                    vehicleCategory={data.kendaraan?.kategori}
                    onDistanceCalculated={(meter) => setTotalJarakMap(meter)}
                  />
                </div>
              </div>
            )}
