...
const HomeMobile = ({ username, roleId, handleLogout, GetNamaDivisi }) => {
  ...
  const IconButton = ({ icon, label, onClick, color, hasNotification, isTop }) => (
    <button onClick={onClick} aria-label={label} className="p-4 relative icon-button-relative">
      <div className="flex flex-col items-center">
        ...
        {hasNotification && (<span className={`absolute ${isTop ? "top-3" : "bottom-3"} right-11 w-2 h-2 bg-red-500 rounded-full transform translate-x-1 ${isTop ? "-translate-y-1/2" : "translate-y-1/2"} animate-pulse`}></span>)}
      </div>
    </button>
  );
  ...
  useEffect(() => {
    ...
    if (id_user) {
      fetch(`${apiUrl}/notif/user/${id_user}`, {headers: {"Cache-Control": "no-cache"}})
        .then((response) => {
