...
import { faArrowLeft, faMapMarkerAlt, faRulerVertical, faEye, faClock, faCalendarAlt, faUser, faArrowRight, faCalendarCheck, faSort, faSortUp, faSortDown } from "@fortawesome/free-solid-svg-icons";
            ...
            {selectedAbsen && (
              <>
                ...
                <div className="flex justify-between items-center mb-4 p-3 bg-gray-50 rounded-lg border">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${selectedAbsen.status === 1 ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                ...
                {(user.id_role === 1 || user.id_role === 4) && (
                  <div className="flex justify-end mt-4">
                    {selectedAbsen.status !== 1 ? (
                      <button onClick={() => handleStatusUpdate(selectedAbsen.id_absen)} disabled={isLoading} className={`px-6 py-2 rounded-lg text-white font-medium ${isLoading ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}>
