...
const AbsenMulai = ({ handleNextStepData }) => {
  ...
  useEffect(() => {
    if (!selectedLocation || !userCoords.latitude || !userCoords.longitude)
      return;
  ...
  useEffect(() => {
    ...
    fetchData("lokasi", setLocations, (data) =>
      data
        .filter((loc) => [1, 2].includes(Number(loc.kategori)))
        .map((loc) => {
          const [latStr, lonStr] = (loc.koordinat || "")
            .split(",")
            .map((s) => s.trim());
  ...
  const checkLocationRadius = (loc) => {
    if (!userCoords.latitude || !userCoords.longitude || !loc?.lat || !loc?.lon)
      return;
  ...
  const filteredLocations = useMemo(() => {
    const allowedLocations = locations.filter((loc) =>
      [1, 2].includes(Number(loc.kategori)),
    );

    // Kadiv hanya boleh pilih lokasi kategori 1 dan 2
    if (isKadiv) return allowedLocations;

    if (
      !jadwal ||
      !Array.isArray(jadwal.lokasi) ||
      jadwal.lokasi.length === 0
    ) {
      return [];
    }
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xl pb-28 space-y-6">
        <section className="bg-white rounded-xl border shadow-sm p-4 space-y-3">
          <div className="flex items-start justify-between gap-3">
            ...
            {fotoMulai && (
              <button
                type="button"
                onClick={() => setFotoMulai(null)}
                className="p-2 px-3 rounded-md border border-red-300 text-red-500 hover:bg-red-50 transition"
            ...
            <>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 py-3 text-md font-semibold rounded-lg border bg-blue-500 text-white hover:bg-blue-600"
                >
                  Putar Kamera
                </button>
                <button
                  type="button"
                  onClick={capture}
                  className="flex-1 py-3 text-md font-semibold rounded-lg bg-green-500 text-white hover:bg-green-600"
                >
                  Ambil Foto
                </button>
              </div>
        ...
        <section className="bg-white rounded-xl border shadow-sm p-4 space-y-3">
          ...
          {!jadwal && !isKadiv && (
            <div className="text-xs text-red-600 font-semibold">
              Anda belum memiliki jadwal kerja hari ini. Silakan hubungi Kepala
              Divisi Anda.
            </div>
          )}
          ...
          <Select
            ...
            placeholder={
              lockLocation ? "Lokasi sudah ditentukan" : "Pilih lokasi"
            }
            className="text-sm"
            onChange={(loc) => {
        ...
        <section className="bg-white rounded-xl border shadow-sm p-4 space-y-3">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">
              {lockShift
                ? "Shift ini telah dijadwalkan oleh Kepala Divisi sesuai pengaturan kerja."
                : "Pilih shift sesuai jadwal kerja yang berlaku dan sesuai."}
            </p>
          </div>

          <Select
            options={shiftList.map((s) => ({ value: s.id, label: s.nama }))}
            value={
              selectedShift
          ...
          {lockShift && (
            <div className="text-[10px] text-gray-400">
              Jika terdapat ketidaksesuaian jadwal, silakan hubungi Kepala
              Divisi Anda.
            </div>
          )}
          ...
          {selectedShift?.detail && (
            <button
              type="button"
              onClick={() => setShowShiftDetail(!showShiftDetail)}
              className="text-xs text-green-600 underline text-left"
            >
              {showShiftDetail
                ? "Sembunyikan detail shift"
                : "Lihat detail shift"}
            </button>
          )}
        ...
        <section className="bg-white rounded-xl border shadow-sm p-4 space-y-2">
          ...
          <textarea
            ...
            className="w-full text-sm p-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-green-400"
