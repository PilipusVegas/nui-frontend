...
const TABS = [
  {
    ...
    description:
      "Jadwal utama yang berlaku secara terus-menerus tanpa batas tanggal. Jadwal ini digunakan sebagai acuan apabila tidak terdapat jadwal lain yang sedang aktif.",
  },
  {
    key: "future",
    label: "Akan Datang",
    ...
    description:
      "Jadwal yang telah dijadwalkan dan akan mulai berlaku pada tanggal tertentu. Selama belum aktif, jadwal ini masih dapat diubah atau dihapus.",
  },
  {
    key: "history",
    label: "Riwayat",
    ...
    description:
      "Jadwal yang sudah selesai dan tidak lagi berlaku. Digunakan hanya untuk melihat data sebelumnya.",
  },
];
...
const CountBadge = ({ count, color }) => {
  ...
  return (
    <span
      className={`ml-2 px-2 rounded-full text-[10px] font-semibold ${color}`}
    >
      {count}
    </span>
  );
...
const DetailPenjadwalan = () => {
  ...
  const fetchDetail = useCallback(async () => {
    try {
      setLoading(true);
      setError(false);
      const res = await fetchWithJwt(`${apiUrl}/jadwal/detail/${id_user}`);
  ...
  const renderPermanentTab = () => {
    ...
    return renderScheduleCard({
      ...
      onEdit: () => navigate(`/penjadwalan/edit/${id_user}?mode=permanent`),
  ...
  const renderTabDescription = () => {
    ...
    return (
      <div
        className={`mt-6 border rounded-xl px-5 py-4 flex gap-4 items-start ${toneMap[tab.tone]}`}
      >
  ...
  const handleEditCurrentSchedule = () => {
    ...
    if (currentSchedule.type === "permanent") {
      ...
      navigate(
        `/penjadwalan/edit/${id_user}?mode=range&startDate=${currentSchedule.tgl_mulai}&endDate=${currentSchedule.tgl_selesai}`,
      );
    }
  };
  ...
  /* ================== DERIVED DATA ================== */
  const currentRange =
    Array.isArray(data?.ongoing) && data.ongoing.length
      ? groupByDate(data.ongoing)[0]
      : null;

  const currentPermanent =
    !currentRange && Array.isArray(data?.permanent) && data.permanent.length
      ? {
          type: "permanent",
  ...
  const scheduleType =
    currentSchedule?.type === "range"
      ? "Rentang Tanggal"
      : currentSchedule?.type === "permanent"
        ? "Jadwal Permanent"
        : "-";
    ...
    <div className="bg-white border border-gray-200 rounded-2xl px-6 py-5 space-y-4">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {lokasi.map((lok, idx) => (
            <span
              key={idx}
              className="inline-flex items-start gap-2 px-3 py-1.5 rounded-md text-xs border border-gray-200 text-gray-700"
            >
              {lok}
            </span>
          ))}
      ...
      {(onEdit || onDelete) && (
        <div className="pt-3 border-t border-gray-100 flex justify-end gap-2">
          {onEdit && (
            <Button size="sm" variant="warning" icon={faEdit} onClick={onEdit}>
              Edit
            </Button>
          )}
          {onDelete && (
  ...
  return (
    <div className="mb-20">
      <SectionHeader
        ...
        subtitle="Kelola Penjadwalan Karyawan"
        onBack={() => navigate(-1)}
        actions={
          <Button
            variant="primary"
            icon={faPlus}
            onClick={() => navigate(`/penjadwalan/tambah-jadwal/${id_user}`)}
          >
            Jadwal
          </Button>
        }
            ...
            <div className="px-6 py-8 bg-gray-50 border-t border-gray-200">
              <div className="flex flex-col items-center text-center gap-3">
                ...
                <p className="text-xs text-gray-500 max-w-md leading-relaxed">
                  Saat ini karyawan belum memiliki jadwal aktif. Jadwal
                  permanent akan digunakan apabila tidak ada jadwal rentang
                  tanggal yang aktif.
                </p>
              </div>
            </div>
          ...
          <div className="px-6 py-5 bg-green-50/40 space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentSchedule.lokasi.map((lok, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-1 text-[11px] rounded-md bg-white font-medium border border-green-100 text-gray-800"
                    >
                      {lok}
                    </span>
                  ))}
            ...
            <div className="pt-4 border-t border-green-100 flex justify-end">
              <Button
                ...
                variant="warning"
                icon={faEdit}
                onClick={handleEditCurrentSchedule}
              >
                Edit Jadwal
              </Button>
            </div>
        ...
        <div className="flex gap-10">
          {visibleTabs.map((tab) => (
            <button
              ...
              className={`pb-2 text-sm font-semibold transition ${activeTab === tab.key ? "border-b-2 border-green-600 text-green-600" : "text-gray-600 hover:text-gray-800"}`}
            >
              <div className="flex items-center gap-1">
                {tab.label}
                {tab.key === "future" && (
                  <CountBadge
                    count={getGroupedCount(data?.future)}
                    color="bg-blue-100 text-blue-700"
                  />
                )}
                {tab.key === "history" && (
                  <CountBadge
                    count={getGroupedCount(data?.history)}
