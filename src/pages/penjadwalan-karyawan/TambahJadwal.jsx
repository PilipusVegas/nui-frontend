...
const TambahPenjadwalanUser = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetchWithJwt(`${apiUrl}/jadwal/${id_user}`, {
                method: "POST",
    ...
    return (
        <div>
            <SectionHeader
                title="Tambah Jadwal"
                subtitle="Jadwalkan Shift dan Lokasi Penugasan."
                onBack={() => navigate(-1)}
            />
            {userInfo && (
                <div className="mt-6 bg-white border border-gray-200 rounded-xl px-5 py-4 shadow-sm">
                    ...
                    <div className="mt-1 text-lg font-semibold text-gray-900">{userInfo.nama}</div>
                    <div className="mt-0.5 flex flex-wrap items-center gap-2 text-sm text-gray-600">
                        <span className="font-medium text-gray-700">{userInfo.role}</span>
            ...
            <div className="mx-auto mt-6 bg-white rounded-xl shadow-sm border w-full px-4 sm:px-6 py-6 space-y-8">
                <div>
                    ...
                    {selectedShift && (
                        <div className="mt-4 border rounded-xl p-4 bg-gray-50">
                            <div className="text-sm font-semibold text-gray-800 mb-3">Detail Jadwal Shift</div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                {selectedShift.detail.map((d, idx) => (
                                    ...
                                    >
                                        <span className="font-medium text-gray-700">{d.hari}</span>
                ...
                <div className="rounded-xl border border-gray-300 bg-white p-4">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-gray-900">Penjadwalan Permanen</p>
                                <p className="mt-0.5 text-xs text-gray-700 leading-relaxed">
                                    {form.permanent === 1
                                        ? "Jadwal ini berlaku permanent dan tidak menggunakan tanggal mulai maupun tanggal selesai."
                                        : "Aktifkan jika jadwal ingin berlaku permanent tanpa tanggal mulai dan tanggal selesai."}
                                </p>
                            </div>
                        </div>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Lokasi Absensi</label>
                    <Select
                    ...
                    {form.lokasi.length > 0 && (
                        <div className="mt-4">
                            <p className="text-xs font-medium text-gray-500 mb-2">Lokasi yang dipilih</p>
                            <div className="flex flex-wrap gap-2">
                                {lokasiList
                                    .filter((l) => [1, 2].includes(l.kategori) && form.lokasi.includes(l.id))
                                    .map((l) => (
