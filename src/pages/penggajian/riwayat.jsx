        ...
        const loadPeriods = async () => {
            try {
                ...
                if (json.data?.length > 1) {
                    // Buang periode terakhir (anggap periode terakhir = periode berjalan)
                    const periodsExceptLast = json.data.slice(0, -1);
                    setPeriodList(periodsExceptLast);

                    // Set periode default ke periode terakhir dari daftar hasil (bukan yang aktif)
                    const lastHistoryPeriod = periodsExceptLast[periodsExceptLast.length - 1];
                    setSelectedPeriod(lastHistoryPeriod.id);
                } else {
                    // Jika cuma ada 1 periode, tetap tampilkan saja
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full mb-4">
                ...
                <div>
                    <Select value={periodList.find((p) => p.id === selectedPeriod) || null} onChange={(option) => setSelectedPeriod(option.id)} isSearchable={false} options={periodList} getOptionLabel={(p) => `Periode ${formatLongDate(p.tgl_awal)} s/d ${formatLongDate(p.tgl_akhir)}`} getOptionValue={(p) => p.id} placeholder="Pilih Periode" className="text-xs font-medium leading-tight" />
                </div>
            ...
            {!loading && !error && !filteredData.length && <EmptyState message="Belum ada riwayat penggajian pada periode ini." />}
            ...
            <Modal isOpen={showInfo} onClose={() => setShowInfo(false)} title="Informasi Riwayat Penggajian">
                <div className="text-sm leading-relaxed space-y-4">
                    <p>
                        Halaman <strong>Riwayat Penggajian</strong> menampilkan data rekap gaji karyawan berdasarkan <strong>periode-periode sebelumnya</strong>. Fitur ini berguna untuk peninjauan, audit, dan dokumentasi historis.
                    </p>
                    ...
                    <ul className="space-y-2">
                        <li className="flex items-start gap-2">
                            ...
                            <span>
                                <strong>Pilih Periode:</strong> Tentukan periode penggajian yang ingin ditampilkan dari daftar yang tersedia.
                            </span>
                        </li>
                        <li className="flex items-start gap-2">
                            <FontAwesomeIcon icon="search" className="text-blue-600 mt-0.5" />
                            <span>
                                <strong>Cari Nama:</strong> Temukan karyawan tertentu dengan cepat tanpa menelusuri daftar panjang.
                            </span>
                        </li>
                        <li className="flex items-start gap-2">
                            <FontAwesomeIcon icon="sync" className="text-orange-600 mt-0.5" />
                            <span>
                                <strong>Muat Ulang Data:</strong> Menyegarkan data riwayat sesuai dengan pembaruan sistem terakhir.
                            </span>
                        </li>
                        <li className="flex items-start gap-2">
                            <FontAwesomeIcon icon="file-excel" className="text-green-700 mt-0.5" />
                            <span>
                                <strong>Unduh Excel:</strong> Mengunduh laporan riwayat gaji untuk keperluan arsip atau pelaporan resmi.
                            </span>
                        </li>
                    </ul>
                    ...
                    <div className="space-y-1">
                        <p>
                            ...
                            <strong>Tabel Riwayat:</strong> Menampilkan daftar karyawan dengan kolom:
                        </p>
                        <ul className="list-disc pl-6 space-y-1">
                            <li><strong>Nama Karyawan</strong> → Identitas karyawan.</li>
                            <li><strong>Total Hari Kerja</strong> → Jumlah hari hadir pada periode tersebut.</li>
                            <li><strong>Total Keterlambatan</strong> → Akumulasi menit keterlambatan.</li>
                            <li><strong>Total Lembur</strong> → Total jam lembur tercatat.</li>
                        </ul>
                    </div>
                    ...
                    <div className="space-y-1">
                        <p>
                            ...
                            <strong>Fitur Pengurutan:</strong> Klik pada judul kolom untuk mengurutkan data (A–Z, Z–A, kecil–besar, besar–kecil).
                        </p>
                    </div>

                    <p className="text-gray-700">
                        Dengan fitur ini, HRD dapat menelusuri riwayat penggajian dengan mudah dan memastikan konsistensi data antarperiode.
                    </p>
                </div>
            </Modal>
        </div>
