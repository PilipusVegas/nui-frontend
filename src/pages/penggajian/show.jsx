...
const DetailPenggajian = () => {
  ...
  const handleRekapData = async () => {
    ...
    addInfoRow(["Total Lemburan", dataUser?.total_overtime + " Menit" || "0"]);
    sheet.addRow([]);
    ...
    const fileName = `Rekap_${dataUser?.nama?.replace(/\s+/g, "_")}_${period.replace(/[^0-9]/g, "")}.xlsx`;
          ...
          <div className="bg-white border border-green-200 rounded-2xl shadow-sm p-5 w-full mx-auto mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 text-center">
              {[
                ...
                { heading: "Total", label: "Lemburan", value: totalLembur || 0 },
                { heading: "Total", label: "Keterlambatan", value: totalKeterlambatan || 0 },
              ].map((item, index) => (
                <div key={index} className="bg-green-50 border border-green-200 rounded-lg px-2 py-2 sm:py-4 shadow-sm">
                  <p className="text-[10px] sm:text-xs text-green-800 font-medium mb-0.5 tracking-tight">
                    {item.heading}
                  </p>
                  <p className="text-xs sm:text-lg font-bold text-green-700 tracking-wide">
                    {item.value}
                  </p>
