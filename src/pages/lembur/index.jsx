...
import { fetchWithJwt, getUserFromToken } from "../../utils/jwtHelper";
  ...
  const Lembur = () => {
    ...
    const [searchTerm, setSearchTerm] = useState(lemburData.lokasi || "");
    ...
    useEffect(() => {
      setSearchTerm(lemburData.lokasi);
    }, [lemburData.lokasi]);
      ...
      <MobileLayout title="Formulir Lembur">
        <form onSubmit={handleSubmitStepOne} className="space-y-5 p-4">
          ...
          <div className="mb-4 relative" ref={dropdownRef}>
              ...
              <input type="text" value={searchTerm}  onChange={(e) => { setSearchTerm(e.target.value); setShowDropdown(true); }} onFocus={() => setShowDropdown(true)} placeholder="Cari atau pilih lokasi" className="w-full p-3 text-sm font-reguler border-2 rounded-md tracking-wide"/>
              {showDropdown && (
                <div className="absolute z-10 bg-white w-full border border-gray-300 mt-1 rounded-md shadow-lg max-h-40 overflow-y-auto">
                  {locations.filter((loc) =>
                    ...
                    <div
                      key={loc.id}
                      onClick={() => {
                        setLemburData((d) => ({ ...d, lokasi: loc.nama, nama_lokasi: loc.id }));
                        setSearchTerm(loc.nama);
                        setShowDropdown(false);
                      }}
            ...
            <div>
              ...
              <div className="space-y-4 text-sm md:text-base mt-4">
                <div>
                  ...
                  <p className="text-gray-900">{user.nama_user || "-"}</p>
                </div>
                <div>
